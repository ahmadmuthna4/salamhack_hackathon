...
@Injectable()
export class ExerciseRepository implements IRepository<Exercise, CreateExerciseDto, GetExerciseDto, UpdateExerciseDto> {
    constructor(@InjectRepository(Exercise) private exerciseRepo: Repository<Exercise>) { }

    async create(createExerciseDto: CreateExerciseDto): Promise<Exercise> {
        const { question, answer_choices, correct_answer, podcast_id } = createExerciseDto;
    ...
    async getById(id: number, query: GetExerciseDto): Promise<Exercise> {
        ...
        const exercise = await this.exerciseRepo.findOne({ select: select ?? [], where: { id }, relations });
    ...
    async delete(id: number): Promise<DeleteResult> {
        ...
        if (deleted.affected === 0) throw new NotFoundException('Exercise not found');
        return deleted;
    }

    async getByPodcastId(podcastId: number, query: GetExerciseDto): Promise<GetAll<Exercise>> {
        const { limit, offset, relations, select } = query;
        const [array, count] = await this.exerciseRepo.findAndCount({
            select: select ?? [],
            take: limit ?? 100,
