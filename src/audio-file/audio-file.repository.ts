...
@Injectable()
export class AudioFileRepository implements IRepository<AudioFile, CreateAudioFileDto, GetAudioFileDto, UpdateAudioFileDto> {
    constructor(@InjectRepository(AudioFile) private audioFileRepo: Repository<AudioFile>) { }
    ...
    async getById(id: number, query: GetAudioFileDto): Promise<AudioFile> {
        ...
        const audioFile = await this.audioFileRepo.findOne({ where: { id }, relations });
        if (!audioFile) throw new NotFoundException('Audio file not found');
        return audioFile;
    }

    async update(id: number, updateDto: UpdateAudioFileDto): Promise<AudioFile> {
        const audioFile = await this.getById(id, {});
        const { file_path } = updateDto;
        audioFile.file_path = file_path ?? audioFile.file_path;
        return await this.audioFileRepo.save(audioFile);
    }

    async delete(id: number): Promise<DeleteResult> {
        const deleted = await this.audioFileRepo.delete({ id });
        if (deleted.affected === 0) throw new NotFoundException('Audio file not found');
        return deleted;
    }

    async getByPodcastId(podcastId: number, query: GetAudioFileDto): Promise<GetAll<AudioFile>> {
        const { limit, offset, relations, select } = query;
        const [array, count] = await this.audioFileRepo.findAndCount({
            take: limit ?? 100,
            skip: offset ?? 0,
