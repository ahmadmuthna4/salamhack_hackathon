...
@Injectable()
export class KeywordRepository implements IRepository<Keyword, CreateKeywordDto, GetKeywordDto, UpdateKeywordDto> {
    ...
    async getById(id: number, query: GetKeywordDto): Promise<Keyword> {
        ...
        const keyword = await this.keywordRepo.findOne({ select: select ?? [], where: { id }, relations });
    ...
    async delete(id: number): Promise<DeleteResult> {
        ...
        if (deleted.affected === 0) throw new NotFoundException('Keyword not found');
        return deleted;
    }

    async getByTranscriptId(transcriptId: string, query: GetKeywordDto): Promise<GetAll<Keyword>> {
